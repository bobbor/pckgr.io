...
(function($) {
	...
	packer.details = function() {
		...
		function renderDetails( url, name ) {
			var ret = packer.configfile.changeConfigFile(url, name);
			var config;
			if(ret.ok) {
				currentURL = url;
				currentName = name;
				...
				if(ret.ok) {
					...
					if(currentName !== name) {
						packer.savefile.changeProjectName(url, currentName);
				...
				packer.boxes.showError(_('boxes.header.error'), _('boxes.list.error.changeProject'), '');
			}
			(!packer.globalConfig.config.lint) && $('button.lint').css({display: 'none'});
		}
		...
		function bindEvents(context) {
			$('h2', context[0])
				...
				.delegate('span', 'click', function(e) {
					...
					$(e.target).bind('keypress keyup', function(ev) {
						...
						if(!isCancel) {
							packer.configfile.setOption('name', e.target.innerText);
							packer.savefile.changeProjectName(currentURL, e.target.innerText);
							packer.list.fillProjectList();	
						} else {
			...
			$('span.URL', context[0]).bind('click', function() {
				var url = packer.filemanager.getFile(this.innerText);
			...
			context.delegate('button.lint','click', function() {
				...
				var ret = packer.configfile.readConfig();
				var list;
				var files;
				var cfg;
					...
					for(prop in list) {
						if(list.hasOwnProperty(prop)) {
							...
							for(i = 0; i < len; i++) {
								...
								ret = packer.filemanager.readContent(new air.File(url).parent.nativePath+'/'+files[i]+'.js');
								if(ret.ok) {
									if(!JSHINT(ret.content, packer.lintConfig.config)) {
					...
					packer.boxes.showDialog({
						...
						cb: function(ok) {
							...
							if(!ok) {
								...
								packer.editor.open(output, null, '', 'Ok');
							}
						}
					});
			...
			$('#fileList').delegate('input', 'click', function() {
				...
				var ret = packer.configfile.setOption(this.name, this.getAttribute('rel'));
				if(!ret.ok) {
					switch(ret.status) {
					case 0:
						packer.boxes.showError(_('boxes.header.error'), _('boxes.details.error.unknownOption', this.name));
						break;
					case 1:
						packer.boxes.showError(_('boxes.header.error'), _('boxes.configfile.error.configwrite'));
						break;
					...
					if(newElm.length && !newElm.find('a.add_package').length) {
						...
						if(diff > 0) {
							if($(e.target).offset().left > togglePos) {
								...
								ret = packer.configfile.switchPackage(oldPos, newPos);
								if(!ret.ok) {
									packer.boxes.showError(_('boxes.header.error'), _('boxes.details.error.movePackage'));
								}
							}
						} else if(diff < 0) {
							if($(e.target).offset().left < togglePos) {
			...
			context.delegate('#fileList a', 'click', function(e) {
				...
				function selectHandler(e) {
					...
					for(var i = 0, len = e.files.length; i < len; i++) {
						ret = packer.configfile.addFile(pack, file.getRelativePath(e.files[i]));
						if(ret.ok) {
							...
							case 0:
								...
								break;
							case 1:
								packer.boxes.showError(_('boxes.header.error'), _('boxes.configfile.error.fileExists', file.getRelativePath(e.files[i])));
								break;
							}
						}
					}
				...
				if($(e.target).is('.add_file')) {
					...
					var file = packer.filemanager.getFile(this.getAttribute('data-url'));
					file = file.parent;
					...
					packer.libhandler.setBasics(e.target.getAttribute('data-package'), 
					...
					if(packer.globalConfig.config.show_warning) {
						packer.boxes.showDialog({
							...
							cb: function(ok) {
								if(ok) {
									var ret = packer.configfile.deletePackage(pack);
									if(ret.ok) {
										if(li.is('.active')) {
											if(li.prev().length && !li.prev().find('a:first').is('.add_package')) {
												...
												packer.configfile.setOption('includeSmallLoader', pack);
											}
										}
										$(e.target).closest('li').remove();
										context.find('p.info span.packs').text(parseInt(context.find('p.info span.packs').text(), 10)-1);
									} else {
										packer.boxes.showError(_('boxes.header.error'),_('boxes.details.error.deletePackage', pack));
									}
								}
							}
						});
						...
						ret = packer.configfile.deletePackage(pack);
						if(ret.ok) {
							if(li.is('.active')) {
								if(li.prev().length && !li.prev().find('a:first').is('.add_package')) {
									li.prev().addClass('active');
					...
					if(packer.globalConfig.config.show_warning) {
						packer.boxes.showDialog({
							...
							cb: function(ok) {
								if(ok) {
									ret = packer.configfile.deleteFile(e.target.getAttribute('data-package'), e.target.getAttribute('rel'));
									if(ret.ok) {
										$(e.target).closest('li').remove();
										context.find('p.info span.files').text(parseInt(context.find('p.info span.files').text(), 10)-1);
									}
			...
			(function() {
				...
				document.body.addEventListener('drop', function(e) {
					...
					if(typeof index !== 'undefined' && content) {
						// we moved internally
						var ret = packer.configfile.switchFile(pack, index, $(e.target).closest('li').index() - 1);
						if(!ret.ok) {
							switch(ret.status) {
							case 0:
								packer.boxes.showError(_('boxes.header.error'), _('boxes.details.error.moveFile'));
								break;
							case 1:
								packer.boxes.showError(_('boxes.header.error'), _('boxes.configfile.error.configwrite'));
								break;
						...
						var timer = window.setInterval(function() {
							...
							if(name === null) {
								packer.boxes.showError(_('boxes.header.error'), _('boxes.details.error.directoryMismatch'));
								clearInterval(timer);
								li.remove();
								return;
							}
							var ending = name.substring(name.lastIndexOf('.')+1, name.length);
							var ret = packer.configfile.addFile(pack, name);
							if(ret.ok) {
								name = name.substring(0, name.lastIndexOf('.'));
								...
								switch(ret.status) {
								case 1:
									packer.boxes.showError(_('boxes.header.error'), _('boxes.configfile.error.fileExists', ret.name));
									break;
								case 0:
									packer.boxes.showError(_('boxes.header.error'), _('boxes.configfile.error.configwrite'));
									break;
								case 2:
									packer.boxes.showError(_('boxes.header.error'), _('boxes.details.error.fileMisMatch', ending));
									break;
								case 3:
									packer.boxes.showError(_('boxes.header.error'), _('boxes.details.error.directoryMismatch'));
								}
