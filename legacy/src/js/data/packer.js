...
(function($){
	...
	packer.packer = function() {
		...
		function getDevLoader(packageName) {
			var result = packer.filemanager.readContent('app:/templates/packer/included_loader.js');
			var content;
			if(result.ok) {
		...
		function getSmallLoader(packageName) {
			var result = packer.filemanager.readContent('app:/templates/packer/included_smallloader.js');
			var content;
			if(result.ok) {
				content = result.content;
		...
		function createPackage(packageName, packageIndex, packedScript) {
			...
			if(packedScript.length) {
				...
				if (!packageIndex) {
					...
					if(result.ok) {
						...
						packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.devLoaderMissing'));
						return;
					}
					...
					if(result.ok) {
						...
						gotErrors = true;
						packer.boxes.hideWaiting();
						packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.smallLoaderMissing'));
						return;
					}
				}
				...
				for(var j = 0, len = cfg.packageList[packageName].length; j < len; j++) {
					...
					if($.inArray(file, cfg.nopack)+1) {
						...
						var ret = packer.filemanager.readContent(file);
						if(ret.ok) {
				...
				packageContent += '}';
				if(!packer.filemanager.writeContent(path.url + '/' + packageName + '.js', packageContent).ok) {
					gotErrors = true;
					packer.boxes.hideWaiting();
					packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.packageWriteFail', packageName));
					return;
				}
			} else {
				gotErrors = true;
				packer.boxes.hideWaiting();
				packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.packerFail'), function(ok) {
					if(ok) {
						var i = errors.length;
						while(i--) {
						...
						packer.editor.open(errors[i].str);
					}
				});
			}
			...
			if(packageIndex === listLength-1 && !gotErrors) {
				...
				packer.boxes.showDialog({
					...
					message: _('boxes.packer.success.packed', cfg.name), 
					type: 'success'
				});
			}
		}
		...
		function compress(packageName, packageIndex) {
			...
			if(!pathToCompressor.exists) {
				packer.boxes.hideWaiting();
				packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.compressorMissing'));
				return;
			}
			...
			if(!pathToExe.exists) {
				packer.boxes.hideWaiting();
				packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.exeMissing', pathToExe.nativePath));
				return;
			}
			...
			try {
				...
				packer.boxes.hideWaiting();
				packer.boxes.showError(_('boxes.header.error'), _('boxes.packer.error.nativeProcessUnavailable'));
			}
		}
		
		/**
		 * @scope packer.closure
		 */
		return {
			/**
			...
			pack: function(config, packagePath, invoke) {
				...
				if(packer.globalConfig.config.lint) {
					packer.boxes.showDialog({
						title: _('boxes.header.waiting'),
					...
					for(prop in list) {
						if(list.hasOwnProperty(prop)) {
							...
							for(i = 0; i < len; i++) {
								...
								ret = packer.filemanager.readContent(path.url+'/'+files[i]+'.js');
								if(ret.ok) {
					...
					if(errorCount !== 0) {
						output = _('boxes.packer.info.validatorResult', errorCount, pkgCount);
					}
					packer.boxes.hideWaiting();
					if(output === '') {
						if(invoke) {
					...
					packer.boxes.showDialog({
						...
						message: _('boxes.packer.info.validator', output+'<br/>'),
