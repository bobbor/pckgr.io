(function($) {
	...
	traydock = function() {
		...
		function hideWhenMinimizing(e) {
			if(e.afterDisplayState === air.NativeWindowDisplayState.MINIMIZED && globalConfig.config.min_tray) {
				e.preventDefault();
		...
		function createMenu(items) {
			...
			function menuItemSelect(e) {
				...
				if(ret.ok) {
					...
					if(ret.ok) {
						packager.pack(ret.config, filemanager.getFile(e.target.data.url));
					...
					boxes.showError(_('boxes.header.error'), _('boxes.tray.error.projectChange', e.target.data.name));
				}
			}
