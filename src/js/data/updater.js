(function($) {
	...
	updater = function() {
		...
		function progress(e) {
			...
			boxes.getInstance().find('#progress span').css({width: prog+'%'}).html(prog+'%');
		}
		...
		function ioerror(e) {
			...
			boxes.showError(_('boxes.header.error'), _('boxes.update.error.ioError'));
		}
		...
		function installWindows(rawFile) {
			...
			var ret = filemanager.writeBinaryContent(dir, rawFile);
			if(ret.ok) {
				...
				boxes.showError(_('boxes.header.error'), _('boxes.update.error.save'));
			}
		}
